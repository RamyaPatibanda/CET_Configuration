...
function FieldConfiguration() {
  ...
  const loadFields = async () => {
    try {
      ...
      setFields(Array.isArray(response) ? response : response?.data || []);
    } catch (loadError) {
  ...
  const handleSave = async (field) => {
    try {
      ...
      if (selectedField) {
        ...
        await fieldConfigurationService.createField(field);
      }
      setFormOpen(false);
      setSelectedField(null);
      await loadFields();
    ...
    <div className="field-configuration-page">
      <div className="field-page-header">
        <div>
          ...
          <p>Manage fields used by the CET configuration system.</p>
        </div>
