...
import { FiEdit2, FiMenu, FiPlus, FiTrash2, FiUsers } from "react-icons/fi";
...
const OPERATORS = {
  ...
  Number: [
    ...
    { value: "GreaterThanOrEqual", label: "Greater Than or Equal" }, { value: "LessThanOrEqual", label: "Less Than or Equal" },
  ],
  Date: [{ value: "Equals", label: "Equals" }, { value: "Before", label: "Before" }, { value: "After", label: "After" }],
  Boolean: [{ value: "Equals", label: "Equals" }],
};

const LOGICAL_OPTIONS = [{ value: "AND", label: "AND" }, { value: "OR", label: "OR" }];
...
function createCondition(groupId = null) {
  return {
    ...
    groupId, fieldId: "", logicalOperator: "AND", operator: "Equals", value: "",
  };
}
...
function toRows(conditions) {
  ...
  return [...conditions]
    ...
    .map((condition, index) => ({
      ...
      logicalOperator: condition.conditionLogicalOperator || condition.logicalOperator || "AND",
...
function RuleForm({ open, rule, nextRuleId, saving, onClose, onSave }) {
  ...
  const changeField = (rowId, value) => {
    ...
    setRows((current) => current.map((row) => row.id === rowId ? { ...row, fieldId: value, operator: firstOperator, value: "" } : row));
    setError("");
  };
  ...
  const selectAll = () => setSelectedRows((current) => current.length === rows.length ? [] : rows.map((row) => row.id));

  const groupSelected = () => {
    if (selectedRows.length < 2) { setError("Select at least two conditions to create a group."); return; }
    setRows((current) => {
      ...
      if (selected.length < 2 || firstSelectedIndex < 0) return current;
      const existingGroupIds = new Set(selected.map((row) => row.groupId).filter(Boolean));
      if (existingGroupIds.size === 1 && selected.every((row) => row.groupId)) { setError("The selected conditions are already in the same group."); return current; }
  ...
  const ungroupSelected = () => {
    if (!selectedRows.length) { setError("Select at least one condition to ungroup."); return; }
    setRows((current) => {
      const selectedSet = new Set(selectedRows);
      const groupedSelection = current.filter((row) => selectedSet.has(row.id) && current.filter((item) => item.groupId && item.groupId === row.groupId).length > 1);
      if (!groupedSelection.length) { setError("Select a condition that belongs to a group before ungrouping."); return current; }
      return current.map((row) => selectedSet.has(row.id) && groupedSelection.some((item) => item.id === row.id) ? { ...row, groupId: null } : row);
    });
    setSelectedRows([]); setError("");
  };
  ...
  const groupedRows = useMemo(() => {
    ...
    rows.forEach((row) => { const groupId = row.groupId || `ungrouped-${row.id}`; if (!groups.has(groupId)) groups.set(groupId, []); groups.get(groupId).push(row); });
    return groups;
  }, [rows]);
  ...
  const handleSubmit = async (event) => {
    ...
    const conditions = [];
    let groupOrder = 0; let previousGroupKey = null;
    for (let index = 0; index < rows.length; index += 1) {
      const row = rows[index];
      if (!row.fieldId || !row.operator || !String(row.value).trim()) { setError("Complete every condition before saving."); return; }
    ...
    setError("");
    await onSave({ ...form, priority: Number(form.priority || 1), conditions });
  };

  return (
    <Dialog open={open} title={rule ? "Edit Rule" : "Create Rule"} className="rule-dialog" onClose={saving ? undefined : onClose}
      ...
      <form id="rule-form" className="rule-form" onSubmit={handleSubmit}>
        ...
        <div className="conditions-header">
          <div><span className="conditions-kicker">Rule logic</span><h3>Conditions</h3><p>Use Logic for the condition-to-condition connector. Group Logic controls how groups are combined.</p></div>
          <div className="condition-toolbar-actions"><Button type="button" variant="secondary" onClick={groupSelected} disabled={selectedRows.length < 2}><FiUsers size={14} />Group {selectedRows.length > 1 ? `(${selectedRows.length})` : ""}</Button><Button type="button" variant="secondary" onClick={ungroupSelected} disabled={!selectedRows.length}>Ungroup</Button><Button type="button" variant="secondary" onClick={addRow}><FiPlus size={14} />Add Condition</Button></div>
        </div>
        <div className="condition-selection-note">{selectedRows.length ? `${selectedRows.length} condition${selectedRows.length === 1 ? "" : "s"} selected` : "Select rows to group or rearrange them with drag and drop."}</div>
        <div className="condition-single-table-wrapper">
          <table className="condition-single-table">
            <thead><tr><th className="condition-check-column"><input type="checkbox" checked={rows.length > 0 && selectedRows.length === rows.length} onChange={selectAll} aria-label="Select all conditions" /></th><th className="condition-brace-column">Group</th><th className="condition-drag-column" /><th>Field</th><th>Operator</th><th>Value</th><th>Logic</th><th className="condition-action-column" /></tr></thead>
            <tbody>
              {rows.map((row) => {
                ...
                return <tr key={row.id} draggable onDragStart={() => setDraggedRow(row.id)} onDragOver={(event) => event.preventDefault()} onDrop={() => moveRow(row.id)} className={isSelected ? "condition-row-selected" : ""}>
                  ...
                  <td><button type="button" title="Delete condition" aria-label="Delete condition" onClick={() => removeRow(row.id)}><FiTrash2 /></button></td>
                </tr>;
              })}
            </tbody>
          </table>
          {!rows.length && <div className="condition-empty"><FiCheckSquare size={18} /><span>No conditions yet. Add a condition to start building the rule.</span></div>}
        </div>
      </form>
    </Dialog>
  );
