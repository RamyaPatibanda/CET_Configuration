...
function RuleConfiguration() {
  ...
  const loadRules = async () => {
    try {
      ...
      setError(
        loadError.message || "Unable to load rules."
      );
    } finally {
      setLoading(false);
    }
  ...
  const openEdit = async (rule) => {
    try {
      ...
      const response = await ruleConfigurationService.getRule(
        rule.ruleId
      );
      ...
      setError(
        loadError.message || "Unable to load rule."
      );
    }
  };
  ...
  const handleSave = async (rule) => {
    try {
      ...
      if (selectedRule) {
        await ruleConfigurationService.updateRule(
          selectedRule.ruleId,
          rule
        );
      } else {
      ...
      setError(
        saveError.message || "Unable to save rule."
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await ruleConfigurationService.deleteRule(
        deleteRule.ruleId
      );
      ...
      setError(
        deleteError.message || "Unable to delete rule."
      );
    }
  };
    ...
    <div className="rule-configuration-page">
      <div className="rule-page-header">
        <div>
          ...
          <p>
            Build allocation rules using the active fields
      ...
      {error && (
        <div className="rule-page-error">
          {error}
        </div>
      )}

      {loading ? (
        <div className="rule-list-message">
          Loading rules…
        </div>
      ) : !rules.length ? (
        <div className="rule-empty-state">
          ...
          <p>
            Create the first rule using your configured fields.
          </p>
      ...
      >
        <div className="rule-delete-confirmation">
          <p>
            The rule <strong>{deleteRule?.ruleName}</strong>
