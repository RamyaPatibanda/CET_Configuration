...
namespace api.DataAccess.RuleConfiguration
{
    public class RuleConfigurationDAL : IRuleConfigurationDAL
    {
        ...
        public RuleConfigurationDAL(IConfiguration configuration, ILogger<RuleConfigurationDAL> logger)
        {
            _logger = logger;
        ...
        public async Task<List<RuleDefinition>> GetRulesAsync()
        {
            try
            {
                ...
                while (await reader.ReadAsync()) rules.Add(MapRule(reader));
                return rules;
            }
            catch (Exception ex) { _logger.LogError(ex, "Error while getting rules."); throw; }
        }
        ...
        public async Task<RuleDefinition?> GetRuleAsync(int ruleId)
        {
            try
            {
                var rule = (RuleDefinition?)null;
                await using var connection = new SqlConnection(_connectionString);
                await using var command = CreateCommand("sproc_GetRule", connection);
                command.Parameters.Add("@aRuleId", SqlDbType.Int).Value = ruleId;
                await connection.OpenAsync();
                await using var reader = await command.ExecuteReaderAsync();
                if (await reader.ReadAsync()) rule = MapRule(reader);
                if (rule is null) return null;
                if (await reader.NextResultAsync())
                {
                    while (await reader.ReadAsync()) rule.Conditions.Add(MapCondition(reader));
                }
                return rule;
            }
            catch (Exception ex) { _logger.LogError(ex, "Error while getting rule {RuleId}.", ruleId); throw; }
        }
        ...
        private static SqlCommand CreateCommand(string procedureName, SqlConnection connection) =>
            new(procedureName, connection) { CommandType = CommandType.StoredProcedure, CommandTimeout = 30 };

        private static void AddRuleParameters(SqlCommand command, int ruleId, string name, string description, int priority, bool active)
        {
            command.Parameters.Add("@aRuleId", SqlDbType.Int).Value = ruleId;
        ...
        private static string SerializeConditions(List<RuleConditionRequest> conditions) =>
            System.Text.Json.JsonSerializer.Serialize(conditions.OrderBy(c => c.ConditionOrder));

        private static RuleDefinition MapRule(SqlDataReader reader) => new()
        {
            RuleId = reader.GetInt32(reader.GetOrdinal("aRuleId")),
            RuleName = reader.GetString(reader.GetOrdinal("tRuleName")),
            Description = reader.IsDBNull(reader.GetOrdinal("tDescription")) ? string.Empty : reader.GetString(reader.GetOrdinal("tDescription")),
        ...
        private static RuleCondition MapCondition(SqlDataReader reader) => new()
        {
